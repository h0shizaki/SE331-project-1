...
function updateKeyword() {
  ...
  if (keyword.value === '' || keyword.value.length === 0) {
    ...
    queryFunction = EventService.getEventsByKeyword(keyword.value.toString(), props.perPage, props.page)
  }
  queryFunction
...
EventService.getEvent().then((res: AxiosResponse<EventItem[]>) => {
  events.value = res.data as EventItem[]
  totalEvent.value = res.headers['x-total-count']
})
    .catch(() => {
...
<template>
  ...
  <main class="flex flex-col items-center">
    ...
    <div class="pagination">
      <RouterLink :to="{'name': 'eventList', query: {page: page-1, perPage: eventPerPage.toString() }}" rel="prev"
                  v-if="page != 1"
                  id="page-prev">
        🔙 Prev Page
      </RouterLink>
